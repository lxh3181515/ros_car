...
#ifdef MPU6500
#define HWST_MAX_PACKET_LENGTH (256)
#endif
...
//					result |= 1 << i;	//Error condition 	//TODO shield this error check 
				}
			}
	}
...
//                                           0, 0, 1};														 
////磁力计方向设置
//static signed char comp_orientation[9] = { 1, 0, 0,
//                                           0, -1, 0,
//                                           0, 0,-1};

static signed char gyro_orientation_dmp[9] = { 1, 0, 0,
																							 0, 1, 0,
																							 0, 0, 1};																						 
////MPU9250自测试
//返回值:0,正常
...
uint8_t run_self_test(void)
{
	...
	if (result == 0x7) 
	{
		...
		*/
		...
		accel[1] /= (8<<16);
		accel[2] /= (8<<16);	
		mpu_set_accel_bias_6500_reg(accel);	//str accel bias to make x,y to zero and z tu +G
...
uint8_t mpu_dmp_init(void)
{
	...
	{	 
		...
		if(res)return 6; 
			res=dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation_dmp));//设置陀螺仪方向
		if(res)return 7; 
...
//返回值:0,正常
//    其他,失败
uint8_t mpu_mpl_get_data(sensordatatyped_t xDataTypes,float* pfDataContainer)
{
	...
	switch(xDataTypes){
		...
		case SENSOR_TYPE_EULER:
			...
			pfDataContainer[0] = (data[0]/q16);
			pfDataContainer[1] = (data[1]/q16);
			pfDataContainer[2] = (data[2]/q16);
			break;
		case SENSOR_TYPE_QUAT:
			inv_get_sensor_type_quat(data,&accuracy,&timestamp);
			pfDataContainer[0] = (data[0]/q30);
			pfDataContainer[1] = (data[1]/q30);
			pfDataContainer[2] = (data[2]/q30);
			pfDataContainer[3] = (data[3]/q30);			
			break;
		case SENSOR_TYPE_GYRO:
			inv_get_sensor_type_gyro(data,&accuracy,&timestamp);
			pfDataContainer[0] = (data[0]/q16)*DEGREE2RADIAN;
			pfDataContainer[1] = (data[1]/q16)*DEGREE2RADIAN;
			pfDataContainer[2] = (data[2]/q16)*DEGREE2RADIAN;	
			break;
		case SENSOR_TYPE_ACCEL:
			inv_get_sensor_type_accel(data,&accuracy,&timestamp);
			pfDataContainer[0] = (data[0]/q16)*DEFAULT_GRAVITY;
			pfDataContainer[1] = (data[1]/q16)*DEFAULT_GRAVITY;
			pfDataContainer[2] = (data[2]/q16)*DEFAULT_GRAVITY;
			break;
		default:break;
	}
	return 0;
...
*/
uint8_t mpu_mpl_get_data_group(float* pfEuler,float* pfQuat,float* pfGyro,float* pfAccel){
	unsigned long sensor_timestamp,timestamp;
	short gyro[3], accel_short[3],compass_short[3],sensors;
	unsigned char more;
	long compass[3],accel[3],quat[4],temperature; 
	...
	/**/
	inv_get_sensor_type_euler(data,&accuracy,&timestamp);
	pfEuler[0] = (data[1]/q16);
	pfEuler[1] = -(data[0]/q16);
	pfEuler[2] = (data[2]/q16);
	/**/
	inv_get_sensor_type_quat(data,&accuracy,&timestamp);
	pfQuat[0] = (data[0]/q30);
	pfQuat[1] = -(data[2]/q30);
	...
	/**/	
	inv_get_sensor_type_gyro(data,&accuracy,&timestamp);
	pfGyro[0] = -(data[1]/q16)*DEGREE2RADIAN;
	pfGyro[1] = (data[0]/q16)*DEGREE2RADIAN;
	pfGyro[2] = (data[2]/q16)*DEGREE2RADIAN;	
	/**/
	inv_get_sensor_type_accel(data,&accuracy,&timestamp);
	pfAccel[0] = -(data[1]/q16)*DEFAULT_GRAVITY;
	pfAccel[1] = (data[0]/q16)*DEFAULT_GRAVITY;
	pfAccel[2] = (data[2]/q16)*DEFAULT_GRAVITY;
	return 0;
}

//uint8_t get_compass(short* pcompass_short){
//	unsigned long sensor_timestamp;
////	short compass_short[3];
//	mpu_get_compass_reg(pcompass_short, &sensor_timestamp);
//}
